...
/**
 * Servlet implementation class acceptDnOrder
 */
@WebServlet("/acceptDnOrder")
public class acceptDnOrder extends HttpServlet {
	...
	private boolean insertTblAccept() {
		...
		try {
			String sql="insert into tblsendAccept(userID,acceptType,money,acceptDate,tblID,workID) values(?,?,?,?,?,?)";
	...
	private boolean updateTblBuy(int tblID) {
		...
		Connection conn=mdb.getconntion();//连接到数据库 
		try {
			String sql="update tblsend set tasked=1 where sendID='"+tblID+"'";
			PreparedStatement ps=conn.prepareStatement(sql);
            ps.executeUpdate();  
            ps.close();
		}catch(Exception e) {

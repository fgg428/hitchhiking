...
/**
 * Servlet implementation class acceptDfOrder
 */
@WebServlet("/acceptDfOrder")
public class acceptDfOrder extends HttpServlet {
	...
	private boolean insertTblAccept() {
		...
		try {
			String sql="insert into tblfoodAccept(userID,acceptType,money,acceptDate,tblID,workID) values(?,?,?,?,?,?)";
	...
	private boolean updateTblBuy(int tblID) {
		...
		Connection conn=mdb.getconntion();//连接到数据库 
		try {
			String sql="update tblfood set tasked=1 where foodID='"+tblID+"'";
			PreparedStatement ps=conn.prepareStatement(sql);
            ps.executeUpdate();  
            ps.close();
		}catch(Exception e) {
	...
	private boolean checkOrder(int tblID) {
		Connection conn=mdb.getconntion();//连接到数据库 
		try {
			String sql="select * from tblfood where foodID='"+tblID+"'";
